...
Rey::Rey(Color color, Coordenada coord) :PiezaGen(color, coord, REY) {}
...
bool Rey::movimientoLegal(int fila, int columna, PiezaGen* casilla) {
	...
	else if (((abs(coord.fila - fila) == 1) and (abs(coord.columna - columna) == 1)) or ((abs(coord.fila - fila) == 1) and (abs(coord.columna - columna) == 0)) or ((abs(coord.fila - fila) == 0) and (abs(coord.columna - columna) == 1))) {	//Comprobacion de movimiento en direccion correcta
